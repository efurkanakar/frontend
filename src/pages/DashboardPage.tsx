/**
 * Dashboard route combining KPI cards, timeline chart, and method breakdowns.
 */
...
const DashboardPage = () => {
  ...
  const cards = useMemo(() => {
    ...
    const shortestOrbit = statsQuery.data?.orbperd?.min
    const longestOrbit = statsQuery.data?.orbperd?.max
    const coolestStar = statsQuery.data?.st_teff?.min
    const hottestStar = statsQuery.data?.st_teff?.max
    ...
    return [
      ...
      {
        title: 'Shortest orbital period',
        value:
          typeof shortestOrbit === 'number'
            ? `${decimalFormatter.format(shortestOrbit)} days`
            : '—',
        hint: 'Fastest orbiting confirmed planet',
        loading: statsQuery.isLoading,
      },
      {
        title: 'Longest orbital period',
        value:
          typeof longestOrbit === 'number'
            ? `${decimalFormatter.format(longestOrbit)} days`
            : '—',
        hint: 'Slowest orbiting confirmed planet',
        loading: statsQuery.isLoading,
      },
      {
        title: 'Coolest host star',
        value:
          typeof coolestStar === 'number'
            ? `${numberFormatter.format(coolestStar)} K`
            : '—',
        hint: 'Minimum stellar effective temperature',
        loading: statsQuery.isLoading,
      },
      {
        title: 'Hottest host star',
        value:
          typeof hottestStar === 'number'
            ? `${numberFormatter.format(hottestStar)} K`
            : '—',
        hint: 'Maximum stellar effective temperature',
        loading: statsQuery.isLoading,
      },
    ]
  }, [
    ...
    statsQuery.data?.disc_year?.max,
    statsQuery.data?.orbperd?.min,
    statsQuery.data?.orbperd?.max,
    statsQuery.data?.st_teff?.min,
    statsQuery.data?.st_teff?.max,
